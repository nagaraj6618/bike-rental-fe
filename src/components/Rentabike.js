...
const Rentabike = () => {
  ...
  const navigate = useNavigate(); // Replace the history object with useNavigate
    ...
    const allRentBikes = async () => {
        try {
          if (!state) {
            window.alert('Please sign in to see all available bikes for rent!');
            navigate('/signin'); // Use useNavigate to navigate
          }
